...
> {
  ...
  shouldComponentUpdate(nextProps: FirestoreMutationWithContextProps) {
    ...
    if(this.props.dependencies && nextProps.dependencies){
      nextProps.dependencies.forEach((prop, index) => {
        if(prop !== this.props.dependencies[index]) 
          propsChanged = true
      })
    }
    return (
...
export class FirestoreMutation extends React.Component<FirestoreMutationProps> {
  render() {
    const { children, path, type, dependencies } = this.props;
    if (path === null) {
